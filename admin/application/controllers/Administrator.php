<?php 
	class Administrator extends CI_Controller{
	  ...
	  // Log in Admin
		...
		public function create_product_category()
		{
			...
			$this->form_validation->set_rules('full_desc', 'Category Description', 'required');
			...
			if($this->form_validation->run() === FALSE){
        		...
        		if (!$this->upload->do_upload('category_image')) {	           
	        		...
	        		$data =  array('upload_data' => $this->upload->data());
					$post_image = $_FILES['category_image']['name'];
					//echo $post_image;die;
	        		$this->Administrator_Model->create_product_category($post_image);
	        		//Set Message					
		...
		public function update_product_category_data()
		{
			...
			if($this->form_validation->run() === FALSE){
    			...
    			if (!$this->upload->do_upload('category_image')) {
	    			...
	    			$data =  array('upload_data' => $this->upload->data());
					$post_image = $_FILES['category_image']['name'];
					$this->Administrator_Model->update_product_category_data($post_image);
					//Set Message
	...
	public function multipleImageUpload($images,$dataID){
		...
		{		
			...
			$data['author'] = $this->Post_Model->get_author();
			...
			if($this->form_validation->run() === FALSE){
			   ...
			   if(!empty($_FILES['userfile']['name'])){
					...
					if(!$this->upload->do_upload('userfile')){
						$errors =  array('error' => $this->upload->display_errors());
						$post_image[0] = 'noimage.png';
					}else{
						$data =  array('upload_data' => $this->upload->data());
						$post_image[0] = $_FILES['userfile']['name'];
					}
				}
				...
				if(!empty($_FILES['desc_img']['name'])){
    			    ...
    			    $config['overwrite']     = TRUE;
					$this->load->library('upload', $config);					
					if(!$this->upload->do_upload('desc_img')){
						$errors =  array('error' => $this->upload->display_errors());
						$post_image[1] = 'noimage.png';
					}else{
						$data =  array('upload_data' => $this->upload->data());
						$post_image[1] = $_FILES['desc_img']['name'];
					}
				}
		...
		public function list_blog($offset = 0){
			// Pagination Config
			$config['base_url'] = base_url(). 'administrator/list_blog/';
	...
	public function update_blog($blog_id = false){
			...
			if($this->form_validation->run() === FALSE){
				...
				if(!$this->upload->do_upload('userfile')){
					...
					$post_image[0] = $this->input->post('post_thumbnail_old_img');
				}else{
					$data =  array('upload_data' => $this->upload->data());
					$post_image[0] = $_FILES['userfile']['name'];
				}
				 ...
				 if ( ! $this->upload->do_upload('userfiledetail'))
                {
                    	...
                    	$data['postimg'] = $this->Administrator_Model->listblogs($this->input->post('id'));
					    $post_image[1] = $this->input->post('post_banner_old_img');;
                }
                else
                {
                        $data = array('upload_data' => $this->upload->data());
                        $post_image[1] = $_FILES['userfiledetail']['name'];
...
public function update_password(){
	...
	public function uploadImage(){
		$config['upload_path']="./assets/images/postsDesc/";
        $config['allowed_types']='gif|jpg|png|jpeg';
        $config['max_size']      = '0';
    	$config['overwrite']     = TRUE;
		$this->load->library('upload',$config);
		...
		if ( ! $this->upload->do_upload('userfile')){
			...
			$result= $this->Administrator_Model->save_upload($title,$image);       
			if ($result == TRUE) {
				$id = $this->db->insert_id();
				$res = $this->db->select()->where('id',$id)->get('post_desc_images')->row_array();   
				//echo $this->db->last_query();die;     
				//echo "<pre>";print_r($res);
				// echo "<tr><td><img src='../assets/images/postsDesc/'".$res['image_path']."></td><td>".$res['image_name']."</td><td>".$res['image_path']."</td></tr>";
