<?php
	class Administrator_Model extends CI_Model
	{
		...
		public function create_product_category($post_image){
	        $data = array(
					'name' => $this->input->post('name'),					
					'full_description' => $this->input->post('full_desc'),
					'type' => 'product',
					'status' => $this->input->post('status'),
		...
		public function update_product_category_data($post_image)
		{
			$data = array(
				'name' => $this->input->post('name'),
				'full_description' => $this->input->post('full_desc'),
		...
		public function create_blog($post_image)
		{
			$slug = url_title($this->input->post('title'), "dash", TRUE);			
			$category_id =$this->input->post('category_id');
			$category = $this->db->select('name')->where('id',$category_id)->get('categories')->row_array();
			...
			$seo_url = $string.'/'.$slug;
			//echo $seo_url;die;
			$rowcount = $this->db->select('seo_url')->where('seo_url',$seo_url)->get('posts')->num_rows();
			if($rowcount>0){
				//echo $rowcount;die;
				$seo_url_new = $seo_url.'-'.date('ymdhis');
				$data = array(
					'title' => $this->input->post('title'), 
					'slug' => $slug,
					'body' => $this->input->post('body'),
...
public function save_upload($title,$image){
	 $data = array( 	 			
        ...
        $result= $this->db->insert('post_desc_images',$data);        
        return $result;
}

public function listBlogDescriptionImages(){
	$query = $this->db->select()->order_by('id','DESC')->get('post_desc_images')->result_array();
	return $query;
}
